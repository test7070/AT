...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr">
	<head>
		...
		<script type="text/javascript">
            ...
            var q_readonlys = [];
            var q_readonlyt = ['txtStraddrno','txtStraddr'];
            var bbmNum = [];
            var bbsNum = [];
            var bbtNum = [];
            ...
            function btnModi() {
            	...
            	//已派車,明細禁止修改
            	Lock(1, {opacity : 0});
            	t_where = "ordeno='"+$('#txtNoa').val()+"' and edittime is not null";
            	q_gt('tranvcce', "where=^^" + t_where + "^^", 0, 0, 0, "");
            }
            ...
            function bbs2bbt(){
            	console.log('function bbs2bbt');
            	//先記錄BBT當前狀態
            	var t_bbt = new Array();
            	for(var i=0;i<q_bbtCount;i++){
            		if($('#txtStraddrno__'+i).val().length==0){
            			continue;
            		}
            		if(q_float('txtMount__'+i)==0 &&  $('#txtContainerno1__'+i).val().length==0 && $('#txtContainerno2__'+i).val().length==0){
            			continue;
            		}
            		t_bbt.push({
            			straddrno:$('#txtStraddrno__'+i).val(),
            			mount : q_float('txtMount__'+i),
            			containerno1 : $('#txtContainerno1__'+i).val(),
            			containerno2 : $('#txtContainerno2__'+i).val()
            		});
            	}
            	//清空BBT
            	for(var i=0;i<q_bbtCount;i++){
            		$('#txtNoq__'+i).val('');
            		$('#txtStraddrno__'+i).val('');
            		$('#txtStraddr__'+i).val('');
            		$('#txtMount__'+i).val('');
            		$('#txtContainerno1__'+i).val('');	
            		$('#txtContainerno2__'+i).val('');		
            	}
            	
            	var curBbtCount = 0;
            	var bbtCount = 0;
            	for(var i=0;i<q_bbsCount;i++){
            		t_addrno = $('#txtAddrno_'+i).val();
            		t_addr = $('#txtAddr_'+i).val();
            		t_mount = q_float('txtMount_'+i);
            		t_isdual = $('#chkIsdual_'+i).prop('checked');
            		
            		if(t_addrno.length==0 || t_mount==0)
            			continue;
        			bbtCount = Math.ceil(t_mount/(t_isdual?2:1));
        			
        			for(var j=0;j<t_bbt.length;j++){
        				if(t_addrno==t_bbt[j].straddrno){
        					if(curBbtCount>=q_bbtCount)
        						$('#btnPlut').click();
        					$('#txtStraddrno__'+curBbtCount).val(t_addrno);
		            		$('#txtStraddr__'+curBbtCount).val(t_addr);
		            		$('#txtMount__'+curBbtCount).val(t_mount<=1?1:t_isdual?2:1);
		            		$('#txtContainerno1__'+curBbtCount).val(t_bbt[j].containerno1);	
		            		$('#txtContainerno2__'+curBbtCount).val(t_bbt[j].containerno2);
        					
        					t_mount -= t_isdual?2:1;
        					curBbtCount++;
        				}
        				if(t_mount<=0)
        					break;
        			...
        			while(t_mount>0){
	            		...
	            		$('#txtStraddr__'+curBbtCount).val(t_addr);
	            		$('#txtMount__'+curBbtCount).val(t_mount<=1?1:t_isdual?2:1);
	            		$('#txtContainerno1__'+curBbtCount).val('');	
	            		$('#txtContainerno2__'+curBbtCount).val('');
		...
		<div class='dbbs'>
			<table id="tbbs" class='tbbs'>
				<tr style='color:white; background:#003366;' >
					...
					<td style="width:350px;"><a id='lblAddr'>起迄地點</a></td>
					<td style="width:60px; text-align: center;">數量</td>
					<td style="width:50px; text-align: center;">雙拖</td>
				</tr>
				<tr  style='background:#cad3ff;'>
					...
					<td><input class="txt" id="chkIsdual.*" type="checkbox" /></td>
				</tr>
			</table>
		</div>
